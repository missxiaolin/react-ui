 ...
 */
export default function useSizes(items: SplitterPanelProps[], containerSize?: number) {
    ...
    const [innerSizes, setInnerSizes] = React.useState<(string | number | undefined)[]>(() => items.map(item => item.defaultSize))
    ...
    return [panelSizes, postPxSizes, postPercentSizes, postPercentMinSizes, postPercentMaxSizes, setInnerSizes] as const
}
