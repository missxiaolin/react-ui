...
import type { CSSInterpolation, CSSObject, TokenType } from '@l/cssinjs'

import { token2CSSVar, useCSSVarRegister, useStyleRegister } from '@l/cssinjs'
...
export type GetDefaultTokenFn<CompTokenMap extends TokenMap, AliasToken extends TokenType, C extends TokenMapKey<CompTokenMap>> = (
    ...
    function genCSSVarRegister<C extends TokenMapKey<CompTokenMap>>(
        ...
        const CSSVarRegister: React.FC<Readonly<CSSVarRegisterProps>> = ({ rootCls, cssVar = {} }) => {
            ...
            useCSSVarRegister(
                ...
                () => {
                    const defaultToken = getDefaultComponentToken<CompTokenMap, AliasToken, C>(component, realToken, getDefaultToken)
                    const componentToken = getComponentToken<CompTokenMap, AliasToken, C>(component, realToken, defaultToken, {
    ...
    function genComponentStyleHook<C extends TokenMapKey<CompTokenMap>>(
        ...
        return (prefixCls: string, rootCls: string = prefixCls): UseComponentStyleResult => {
            ...
            const wrapSSR = useStyleRegister({ ...sharedConfig, path: [concatComponent, prefixCls, iconPrefixCls] }, () => {
                ...
                const defaultComponentToken = getDefaultComponentToken<CompTokenMap, AliasToken, C>(component, realToken, getDefaultToken)

                const componentCls = `.${prefixCls}`
                const componentToken = getComponentToken<CompTokenMap, AliasToken, C>(component, realToken, defaultComponentToken, {
                    deprecatedTokens: options.deprecatedTokens
                })
